...
> 本文档根据我们前面几轮对话整理而成，重点服务于“颜色科学中的光谱重建”，即从 CIE XYZ、linear RGB 或相机响应估计物体表面反射率 `R(λ)`。文档分为三部分：UEF 数据集总览、面向光谱重建的数据选择建议、哑光色卡与高光色卡的区别。
...
```text
基于 UEF 反射谱数据库，构建从色度/相机响应到可见光表面反射率的重建模型，
并比较 PCA、Wiener、局部加权和神经网络方法在标准色片、校准靶和真实材料上的
光谱误差与色度误差。
```
...
如果后面要把数据下载到本地并写代码处理，建议按以下结构组织：

```text
uef_spectral_data/
  raw/                  # UEF 或 Zenodo 下载的原始文件
  extracted/             # 解压后的 ASCII / MATLAB / README 文件
  processed/
    wavelength_400_700_10nm/
      munsell_matt.csv
      munsell_glossy_all.csv
      agfa_it872.csv
      natural_colors.csv
      paper_spectra.csv
      forest_colors.csv
...
建议在 `dataset_manifest.csv` 中记录：

| 字段 | 含义 |
|---|---|
| dataset_name | 数据集名称 |
| source_url | UEF 页面或 Zenodo 页面 |
| object_type | 色卡、纸张、自然样本、植被等 |
| spectrum_type | reflectance / absorbance / illumination / synthetic |
| wavelength_min_nm | 最小波长 |
| wavelength_max_nm | 最大波长 |
| wavelength_step_nm | 波长间隔 |
| sample_count | 样本数量 |
| recommended_role | train / validation / test / external_test / not_primary |
| license_note | 非商业、CC BY-NC-SA、需联系作者等 |
