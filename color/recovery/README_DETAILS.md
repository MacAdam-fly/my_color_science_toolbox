...
默认 spectrum recovery method 是 `gaussian`。这适合作为 emission-like 光谱恢复的
默认入口；如果需要自由度更高的基线解，应显式传入
`BoundedLeastSquaresOptions(...)` 或 `method="bounded_least_squares"`。
...
默认 reflectance recovery method 是 `burns2019`。它是无数据库先验的 bounded
reflectance 方法，比平滑最小二乘更符合物体反射率的默认语义。
