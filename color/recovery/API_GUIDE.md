...
`color.recovery` 当前实现 bounded smooth least-squares、Meng 2015、reflectance
PCA 和 convex dictionary recovery。Smits、basis 等方法尚未实现；方法注册表会继续作为扩展入口。
...
- Spectrum recovery 当前只支持 `"bounded_least_squares"`。
...
- 三种方法都不恢复“唯一真实光谱”。应同时检查恢复曲线、目标响应闭合误差和所选先验是否符合任务语义。
