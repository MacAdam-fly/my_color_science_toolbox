...
CAM02/CAM16 均匀空间继承底层色貌模型的参考域规则：`XYZ`、`XYZ_w`、
`Y_b` 必须使用同一个亮度标度。项目默认推荐 `Y=100`，也就是
`XYZ_w=D65_XYZ`、`Y_b=20` 这类写法。如果显式使用
`XYZ_w=D65_XYZ * 2`，代码允许，但这表示进入 `Y_w=200` 的参考域；
此时输入 `XYZ` 也应在同一参考域中，若想保持相同背景相对亮度，
`Y_b` 应同步从 `20` 调整为 `40`。`L_A` 是适应场亮度参数，不会随着
这个数值参考域自动缩放。

这和 D65 色适应不是同一件事：`utils.scale` 只解决 `[0, 1]` 与
`Y=100` 之类的数值尺度，`adapt_to_D65(...) / adapt_from_D65(...)` 才解决
参考白点变化。
