...
`color.datasets` 是 `color/data/` 的公共读取层，负责数据集注册、懒加载、缓存、文件解析和少量公式生成数据。应用代码应优先使用这里的 API，而不是直接读取原始数据文件。
...
所有加载函数返回 `dict[str, numpy.ndarray]`。通常第一列为 `wavelength`，其余键取决于数据集，例如 `X/Y/Z`、`l/m/s`、`spd`、`radiance`、色卡色块名或 Munsell 字段名。
...
_registry.get(category, name, **kwargs)
        │
        ├── 文件数据集：read_csv/read_xlsx/read_xls
        ├── 自定义解析：compute_fn
        └── 公式数据集：compute_fn
```
...
| `blackbody` | 公式 | Planck 黑体辐射，参数 `temperature`、`wavelength_nm` |
| `daylight` | 公式 | CIE D-series daylight，参数 `cct`、`wavelength_nm` |

```python
import numpy as np

wl = np.arange(400, 701, 5.0)
...
| `description` | 人类可读描述 |
| `source` | 数据来源 |
| `file_path` | 文件数据集路径；公式数据集为 `None` |
| `computed` | 是否通过 `compute_fn` 生成或自定义解析 |
| `compute_fn` | 自定义解析函数或公式函数；特殊文件也使用这个入口 |
| `columns` | 显式列名，优先级最高 |
...
需要自定义解析的特殊文件不要把 callable 放入 `metadata`，而应注册为 `computed=True` 并提供 `compute_fn`。这样公式生成数据和特殊文件解析共用同一个扩展入口。
...
def my_compute(wavelength_nm=None, scale=1.0):
    ...
    return {"wavelength": wavelength_nm, "value": values}
...
文件数据集和计算数据集都会按调用参数缓存，因此同一 Excel 文件用不同 `sheet` 读取时不会互相污染。传入 numpy 数组时，缓存键使用数组 dtype、shape 和 bytes 表示。
