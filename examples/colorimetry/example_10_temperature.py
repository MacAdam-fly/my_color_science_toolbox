...
def _plot_loci_comparison(
    ...
    with plot_style("journal"):
        ...
        plot_lines(
            [
                (daylight_xy[:, 0], daylight_xy[:, 1]),
                (planckian_xy[:, 0], planckian_xy[:, 1]),
            ],
            ax=ax,
            labels=["CIE D daylight locus", "Planckian locus, Duv=0"],
            colors=["tab:blue", "0.20"],
            linestyles=["-", "--"],
            linewidth=2.0,
        )
        ...
        for name, cct in named_ccts.items():
            ...
            plot_segments(
                np.array([[daylight_point, planckian_point]]),
                ax=ax,
                colors="0.65",
                linestyles=":",
                linewidth=1.0,
            ...
            plot_points(daylight_point, ax=ax, colors="tab:blue", sizes=58)
            plot_points(planckian_point, ax=ax, colors="0.20", markers="x", sizes=46)
            plot_labels(
                daylight_point,
                [f"{name}\n{cct:.0f} K"],
                ax=ax,
...
def _plot_duv_offsets(output_dir: Path) -> None:
    ...
    with plot_style("journal"):
        ...
        for cct, color in zip(ccts, colors):
            ...
            labelled = [(duv, point) for duv, point in zip(duvs, uv) if duv in {-0.02, 0.0, 0.02}]
            if labelled:
                plot_labels(
                    [point for _duv, point in labelled],
                    [f"{duv:+.2f}" for duv, _point in labelled],
                    ax=ax,
                    offset=(6, 4),
                    color=tuple(color),
                    grid=False,
...
def _plot_method_comparison(
    ...
    with plot_style("journal_double"):
        ...
        plot_lines(
            ...
            ax=ax0,
            labels=["McCamy CCT", "Robertson CCT", "Ohno CCT"],
            marker="o",
            title="CCT Estimates for the Same xy Samples",
            ylabel="CCT (K)",
        )
        ...
        plot_bars(
            ...
            ax=ax1,
            group_labels=["McCamy - Ohno", "Robertson - Ohno"],
            ylabel="CCT difference (K)",
        )
        ...
        plot_bars(
            ...
            ax=ax2,
            labels=sample_names,
            group_labels=["Robertson Duv", "Ohno Duv"],
            ylabel="Duv",
        )
...
def _plot_mired_curve(output_dir: Path) -> None:
    ...
    with plot_style("journal"):
        ...
        plot_lines(
            ...
            colors="tab:purple",
            linewidth=2.0,
            title="CCT and Mired Relationship",
            xlabel="CCT (K)",
            ylabel="Mired",
        )
        points = np.column_stack([named, named_mired])
        plot_points(points, ax=ax, colors="tab:purple", sizes=48)
        plot_labels(
            points,
            [f"{cct_value:.0f} K\n{mired_value:.1f} mired" for cct_value, mired_value in zip(named, named_mired)],
            ax=ax,
            offset=(8, 8),
            grid=False,
        )
